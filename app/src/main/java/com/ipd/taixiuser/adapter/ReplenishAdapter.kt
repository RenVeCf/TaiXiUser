 ...
 */
class ReplenishAdapter(val context: Context, private val list: List<ReplenishBean.PurchasegoodsBean>?, private val onCartNumChangeListener: ProductOperationView.OnCartNumChangeListener, private val itemClick: (info: ReplenishBean.PurchasegoodsBean) -> Unit) : RecyclerView.Adapter<ReplenishAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        holder.itemView.fox_operation_view.setOnCartNumChangeListener(object : ProductOperationView.OnCartNumChangeListener {
            override fun onNumChange(lastNum: Int, num: Int) {
                onCartNumChangeListener.onNumChange(lastNum, num)
                info.chooseNum = num
            }
        })
