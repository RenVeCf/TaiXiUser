...
import com.ipd.taixiuser.adapter.MatterForwardAdapter
...
class MatterForwardFragment : ListFragment<BaseResult<MatterResultBean>, MatterBean>() {

    companion object {
        fun newInstance(typeId: Int): MatterForwardFragment {
            val fragment = MatterForwardFragment()
            val bundle = Bundle()
    ...
    private var mAdapter: MatterForwardAdapter? = null
    override fun setOrNotifyAdapter() {
        if (mAdapter == null) {
            mAdapter = MatterForwardAdapter(mActivity, data) {
                //itemClick
