package com.ipd.jumpbox.jumpboxlibrary.activity;
...
public class ClipActivity extends Activity implements OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Bitmap bitmap = BitmapUtils.convertToBitmap(path, 600, 600);
        if (bitmap == null) {
            Toast.makeText(this, "图片加载失败", Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        if (v.getId() == R.id.rl_botom_clip) {
            ...
            new Thread(new Runnable() {
                ...
                public void run() {
                    ...
                    File destDir = new File(Environment.getExternalStorageDirectory() + "/shangpin/");
                    if (!destDir.exists()) {
                        destDir.mkdirs();
                    }
                    pathfile = Environment.getExternalStorageDirectory() + "/shangpin/"
                            + System.currentTimeMillis() + ".png";
                    BitmapUtils.savePhotoToSDCard(bitmap, pathfile);
